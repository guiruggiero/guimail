...
- `addToBudget` — writes to the budget Google Sheet via Guiddleware's `POST /sheets/values` (issuer→row mapping stays here, since there's no "the one spreadsheet" from Guiddleware's point of view); accepts an optional `last4` to disambiguate issuers with multiple cards (Chase today); also creates a Settle Up expense automatically (split equally, via Guiddleware) if the issuer is Capital One; returns `toolResult.link` as `{url, label}` for a "View Budget Spreadsheet" link
- `addToSettleUp` — calls Guiddleware's `POST /settleup/expenses`, which owns validation; this handler only formats the reply text based on the response; accepts optional `splitEqually` (boolean, 50/50 with Georgia) or `guiOwes`/`georgiaOwes` (exact uneven split, must sum to `amount`) — mutually exclusive; also accepts optional `date`, `paidBy` (`"gui"` default or `"georgia"`), and a free-form `currency`. Scoped to Gui and Georgia only — no arbitrary friend splits: an optional `otherPeople` (array of names) forces a solo entry with those names appended to `details` as a note whenever anyone besides Gui/Georgia is involved, even alongside Georgia, ignoring `splitEqually`/`guiOwes`/`georgiaOwes`/`paidBy`; no return link, since Settle Up has no public per-expense URL
...
- `guiddleware.js` — axios client for the shared Guiddleware service: `createExpense(payload)` (Settle Up), `createCalendarEvent(payload)`, `getFlightAwareUrl(flightNumber)`, `createTask(payload)`, `updateSheet(payload)`, `createTrelloCard(payload)`, `searchTrelloCards(query)`, `updateTrelloCard(id, payload)`
- `langfuse.js` — eagerly initialized Langfuse client, `getPrompt(name)`

Google Calendar, FlightAware, and Google Sheets clients used to live here too — they all moved to the shared `guiddleware` repo since GuiDo needs some of the same integrations (and Sheets, while Guimail-only today, is just as generic an integration as the rest); see `guiddleware.js` above and `guiddleware`'s own `tools/CLAUDE.md`. Guimail no longer holds any Google service-account credentials at all — `googleAuth.js`/`service-account-key.json` were deleted once Sheets moved, since nothing local needed them anymore.
...
`GOOGLE_SHEET_ID` is Guimail's own config (which spreadsheet its budget tool writes to), passed to Guiddleware per-request — Guiddleware has no hardcoded spreadsheet, unlike Settle Up/Calendar where the IDs are genuinely shared, global config.
...
`agent/scripts/` — not deployed with the function. `promptSync.js` (Langfuse pull/push) only.
