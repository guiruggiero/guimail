...
- `addToBudget` — writes to the budget Google Sheet via Guiddleware's `POST /sheets/values` (issuer→row mapping stays here, since there's no "the one spreadsheet" from Guiddleware's point of view); accepts an optional `last4` to disambiguate issuers with multiple cards (Chase today); also creates a Splitwise expense automatically (via Guiddleware) if the issuer is Capital One; returns `toolResult.link` as `{url, label}` for a "View Budget Spreadsheet" link
