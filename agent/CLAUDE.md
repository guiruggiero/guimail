...
- `guiddleware.js` — axios client for the shared Guiddleware service: `createExpense(payload)` (Splitwise), `createCalendarEvent(payload)`, `getFlightAwareUrl(flightNumber)`, `createTask(payload)`, `updateSheet(payload)`
