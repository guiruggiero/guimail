...
- `addToCalendar` — creates events via the Google Calendar API; routes to `GOOGLE_CAL_DEFAULT_ID` or `GOOGLE_CAL_SHARED_ID` based on the `calendar` arg ("default"/"shared"); timed events use `transparency: "opaque"` (busy), all-day events use `transparency: "transparent"` (free); all-day detected by absence of `T` in the `start` string; accepts optional `flightNumber` (IATA) and calls FlightAware AeroAPI to embed a tracking link (best-effort, failures captured in Sentry); accepts optional `reminders` (array of `{method: "email"|"popup", minutes}`, clamped to 5 entries and 0–40320 minutes) overriding the calendar's default reminders; accepts optional `isSpecialProject` boolean that sets `colorId: "10"` (Basil) when true; returns `toolResult.link` as `{url, label}` for a "View in Google Calendar" link
...
- `addToSplitwise` — creates a Splitwise expense via `createSoloExpense`, `createSharedExpense`, or `createExpenseFromShares`; accepts optional `splitWith` (array of friend names) for an equal split, or `owedAmounts` (array of `{name, owed}`) for an uneven split — still a single payer either way, from `paidBy` (name of payer, defaults to Gui via `SPLITWISE_ID_GUI`); the two are mutually exclusive, `owedAmounts` takes precedence if both are somehow present; also accepts optional `date` (backdates/postdates the expense; bare dates are normalized to noon UTC) and a free-form `currency` (no internal validation - Splitwise's `create_expense` rejects an invalid code on its own); resolves names via `getFriendRegistry()` (local `SPLITWISE_FRIENDS` registry only); falls back to a solo expense with a note if any name can't be resolved or (for `owedAmounts`) the amounts don't sum to the total; returns `toolResult.link` as `{url, label}` for a "View in Splitwise" link
...
- `splitwise.js` — `getFriendRegistry` (local `SPLITWISE_FRIENDS` registry), `createSoloExpense`, `createSharedExpense` (equal split, delegates to `createExpenseFromShares`), `createExpenseFromShares` (arbitrary per-person `paid`/`owed`), `createExpenseWithGeorgia`; `checkSplitwiseError`, `normalizeDate`, and `splitEqual` are internal helpers
