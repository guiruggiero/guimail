...
- `addToSplitwise` — creates a Splitwise expense via `createSoloExpense`, `createSharedExpense`, or `createExpenseFromShares`; accepts optional `splitWith` (array of friend names) and `paidBy` (name of payer, defaults to Gui via `SPLITWISE_ID_GUI`) for an equal split, or `shares` (array of `{name, paid, owed}`) for uneven splits and/or multiple payers — the two are mutually exclusive, `shares` takes precedence if both are somehow present; also accepts optional `date` (backdates/postdates the expense; bare dates are normalized to noon UTC) and a free-form `currency` validated against `getSupportedCurrencies()`; resolves names via `getFriendRegistry()` (async: local `SPLITWISE_FRIENDS` nicknames/aliases first, then live Splitwise friends); falls back to a solo expense with a note if any name can't be resolved or (for `shares`) the paid/owed amounts don't sum to the total; returns `toolResult.link` as `{url, label}` for a "View in Splitwise" link
...
- `splitwise.js` — `getFriendRegistry` (async; merges local `SPLITWISE_FRIENDS` aliases with a TTL-cached live `/get_friends` lookup), `getSupportedCurrencies` (cached `/get_currencies` lookup), `createSoloExpense`, `createSharedExpense` (equal split, delegates to `createExpenseFromShares`), `createExpenseFromShares` (arbitrary per-person `paid`/`owed`), `createExpenseWithGeorgia`; `checkSplitwiseError`, `normalizeDate`, `splitEqual`, and the static/live friend registry helpers are internal
